...
func _process(delta: float) -> void:
	...
	if(animation):
		get_node("Goggles").position.y-=delta*90
		get_node("Goggles").scale.x+=delta*0.6
		get_node("Goggles").scale.y+=delta*0.6
		get_node("Goggles").modulate.a-=delta*1.5
		if(get_node("Goggles").modulate.a<0):
			queue_free()
...
func buy()->void:
	...
	if(player.money>price-1):
		player.money-=price
		player.goggles=true
		get_node("/root/Game").goggles=true
		get_node("/root/Game/World/Kanvas/UI/Goggles").visible=true
