...
func _process(delta: float) -> void:
	...
	if(animation):
		get_node("Bomba").position.y-=delta*90
		get_node("Bomba").scale.x+=delta*0.6
		get_node("Bomba").scale.y+=delta*0.6
		get_node("Bomba").modulate.a-=delta*1.5
		if(get_node("Bomba").modulate.a<0):
			queue_free()
